...
//        System.out.println(x1.getColumnDimension());
//        System.out.println(t.getRowDimension());
//        System.out.println(t.getColumnDimension());
        ...
        double v = (basisDegree + 1) * (Math.pow(dx, basisDegree));
        D = matLab.divMatrix(matLab.diff(matLab.eye(nt), basisDegree + 1), Gamma.gamma(v));
        d = matLab.divMatrix(new Matrix(matLab.diff(matLab.eye(nt), basisDegree + 1)), Gamma.gamma(v));
        base = matLab.multMatrix(matLab.multMatrix(P, matLab.transpose(D)), Math.pow(-1, basisDegree + 1));
        Base = p.times(d.transpose()).times(Math.pow((-1), basisDegree + 1));
        ...
        System.out.println(SK[0].length);
        MASK = matLab.lessThan(X2, SK);
        base = Base.times(new Matrix(MASK)).getArray();
