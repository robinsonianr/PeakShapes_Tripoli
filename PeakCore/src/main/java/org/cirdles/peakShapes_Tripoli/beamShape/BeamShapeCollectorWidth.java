...
public class BeamShapeCollectorWidth {
    ...
    public void calcBeamShapeCollectorWidth() {
        ...
        this.beamMassInterp = MatLab.linspace(xLower, xUpper, nInterp);
        ...
        for (int iMass = 0; iMass < numMagnetMasses; iMass++) {
            Matrix massesInCollector = (MatLab.greaterOrEqual(beamMassInterp, peakMeas.getCollectorLimits().get(iMass, 0))).arrayTimes(MatLab.lessOrEqual(beamMassInterp, peakMeas.getCollectorLimits().get(iMass, 1)));
        ...
        // Smoothing spline
//        double lambda = 1e-11;
//        Matrix matrixD = MatLab.diff(MatLab.eye((int) (beamKnots + basisDegree)), orderDiff);
...
//        Matrix wtsAugmented = MatLab.blkDiag(WData, MatLab.eye((int) beamKnots + basisDegree - orderDiff));
...
//        Matrix beamNNPspl = MatLab.solveNNLS(test3, test4);
        ...
        this.leftBoundary = MatLab.find(leftThesholdChange, 1, "last").get(0, 0) + 1;
        ...
        this.rightBoundary = MatLab.find(rightThesholdChange, 1, "first").get(0, 0) + maxBeamIndex;
